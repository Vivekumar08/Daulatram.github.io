...
router.post('/delete_Founder_About_data/:id', async(req, res) => {
    try {
        ...
        if (type === "para") {
            ...
            const delete_user = await Founder.findOneAndDelete({ _id: req.params.id });
...
router.post('/delete_Mission_About_data/:id', async(req, res) => {
    try {
        ...
        if (type === "para") {
            ...
            const delete_user = await Mission.findOneAndDelete({ _id: req.params.id });
            const img = delete_user.img_data.file_path
            await unlinkAsync(img[0].file_path1)
            res.status(202).json(delete_user + "User deleted")
        }
...
router.post('/delete_VicePrincipal_About_data/:id', async(req, res) => {
    try {
        ...
        if (type === "para") {
            ...
            const delete_user = await VicePrincipal.findOneAndDelete({ _id: req.params.id });
            const img = delete_user.img_data.file_path
            await unlinkAsync(img[0].file_path1)
            res.status(202).json(delete_user + "User deleted")
        }
...
router.delete('/delete_Physics_Gallery/:id', async(req, res) => {
    const delete_user = await Physics_Gallery.findOneAndDelete({ _id: req.params.id });
    console.log(delete_user.file_path)
    await unlinkAsync(delete_user.file_path)
    res.status(200).json(delete_user + "User deleted")
...
router.delete('/delete_Zoology_Gallery/:id', async(req, res) => {
    const delete_user = await Zoology_Gallery.findOneAndDelete({ _id: req.params.id });
    console.log(delete_user.file_path)
    await unlinkAsync(delete_user.file_path)
    res.status(200).json(delete_user + "User deleted")
...
router.delete('/delete_Sanskrit_Gallery/:id', async(req, res) => {
    const delete_user = await Sanskrit_Gallery.findOneAndDelete({ _id: req.params.id });
    console.log(delete_user.file_path)
    await unlinkAsync(delete_user.file_path)
    res.status(200).json(delete_user + "User deleted")
...
router.delete('/delete_Psychology_Gallery/:id', async(req, res) => {
    const delete_user = await Psychology_Gallery.findOneAndDelete({ _id: req.params.id });
    console.log(delete_user.file_path)
    await unlinkAsync(delete_user.file_path)
    res.status(200).json(delete_user + "User deleted")
    ...
    try {
        ...
        if (new_) {
            if (date_regex.test(date_exp)) {
                const user = new Archives_Notice({ date, title, date_exp, new_, file_path: file_path, file_mimetype: file_mimetype });
                await user.save();
                return res.status(200).json({ message: "Details Saved Successfully " })
            } else {
            ...
            const user = new Archives_Notice({ date, title, new_, file_path: file_path, file_mimetype: file_mimetype });
            await user.save();
            return res.status(200).json({ message: "Details Saved Successfully " })
        }
    } catch (err) {
...
router.delete('/delete_archives_notice/:id', async(req, res) => {
        const delete_user = await Archives_Notice.findOneAndDelete({ _id: req.params.id });
        if (delete_user.file_mimetype === 'text/link') {
            console.log(delete_user.file_mimetype)
            res.status(200).json(delete_user + "User deleted")
        } else {
    ...
    try {
        ...
        if (new_) {
            if (date_regex.test(date_exp)) {
                ...
                if (user.length >= 12) {
                    return res.status(402).json({ message: "Delete some previous bulletin" })
                } else {
                    const user_save = new Bulletins_Notice({ date, title, date_exp, new_, file_path: file_path, file_mimetype: file_mimetype });
            ...
            if (user.length >= 12) {
                ...
                const user_save = new Bulletins_Notice({ date, title, new_, file_path: file_path, file_mimetype: file_mimetype });
                await user_save.save();
                return res.status(200).json({ message: "Details Saved Successfully " })
            }
...
router.delete('/delete_Bulletins_notice/:id', async(req, res) => {
        try {
            const delete_user = await Bulletins_Notice.findOneAndDelete({ _id: req.params.id });
            if (delete_user.file_mimetype === 'text/link') {
                res.status(200).json(delete_user + "User deleted")
            } else {
                await unlinkAsync(delete_user.file_path)
...
router.post('/Public_notice_add_link', async(req, res) => {
    try {
        ...
        if (new_) {
            if (date_regex.test(date_exp)) {
                const user = new Public_Notice({ date, title, date_exp, new_, file_path: file, file_mimetype: 'text/link' });
                await user.save();
                return res.status(200).json({ message: "Details Saved Successfully " })
            } else {
            ...
            const user = new Public_Notice({ date, title, new_, file_path: file, file_mimetype: 'text/link' });
            await user.save();
            return res.status(200).json({ message: "Details Saved Successfully " })
        }
    } catch (err) {
...
router.post(
    ...
    async(req, res) => {
        try {
            ...
            if (new_) {
                if (date_regex.test(date_exp)) {
                    const user = new Public_Notice({ date, title, date_exp, new_, file_path: path, file_mimetype: mimetype });
                    await user.save();
                    console.log("Details Saved Successfully")
                    return res.status(200).json({ message: "Details Saved Successfully " })
                } else {
                ...
                const user = new Public_Notice({ date, title, new_, file_path: path, file_mimetype: mimetype });
                await user.save();
                console.log("Details Saved Successfully")
                return res.status(200).json({ message: "Details Saved Successfully " })
            }
...
router.delete('/delete_Student_notice/:id', async(req, res) => {
    const delete_user = await Student_Notice.findOneAndDelete({ _id: req.params.id });
    if (delete_user.file_mimetype === 'text/link') {
        console.log(delete_user.file_mimetype)
        res.status(200).json(delete_user + "User deleted")
    } else {
...
router.post('/Student_notice_add_link', async(req, res) => {
    try {
        ...
        if (new_) {
            if (date_regex.test(date_exp)) {
                const user = new Student_Notice({ date, title, date_exp, new_, file_path: file, file_mimetype: 'text/link' });
                await user.save();
                return res.status(200).json({ message: "Details Saved Successfully " })
            } else {
            ...
            const user = new Student_Notice({ date, title, new_, file_path: file, file_mimetype: 'text/link' });
            await user.save();
            return res.status(200).json({ message: "Details Saved Successfully " })
        }
    } catch (err) {
...
router.post(
    ...
    async(req, res) => {
        try {
            ...
            if (new_) {
                if (date_regex.test(date_exp)) {
                    const user = new Student_Notice({ date, title, date_exp, new_, file_path: path, file_mimetype: mimetype });
                    await user.save();
                    console.log("Details Saved Successfully")
                    return res.status(200).json({ message: "Details Saved Successfully " })
                } else {
                ...
                const user = new Student_Notice({ date, title, new_, file_path: path, file_mimetype: mimetype });
                await user.save();
                console.log("Details Saved Successfully")
                return res.status(200).json({ message: "Details Saved Successfully " })
            }
