...
function IQAC() {
  ...
  const fetchdata = async () => {
    const response = await fetch("/IQAC");
  ...
  const del_para = async (id, pid, type) => {
    try {
      ...
      console.log(id, arr);
      const response = await fetch(`/delete_IQAC_para/${id}`, {
        method: "POST",
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(`/delete_IQAC/${id}`, {
      method: "POST",
    });
    await response.json();
    if (response.status === 200) {
  ...
  const handleSubmit_link = async (id, link) => {
    try {
      console.log(link);
      await axios.post(`/IQAC_add_link/${id}`, {
        link: link,
      });
      setCaption("");
      setAuth(true);
