...
const Administration = () => {
  ...
  const del_para = async (id, pid, type) => {
    try {
      ...
      console.log(id, arr);
      const response = await fetch(`/delete_Administration_para/${id}`, {
        method: "POST",
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(`/delete_Administration/${id}`, {
      method: "POST",
    });
    await response.json();
    if (response.status === 200) {
  ...
  const delete_file = async (id, pid, file_path1) => {
    ...
    await axios.post(
      `/delete_pdf_link_Administration_fac/${id}`,
  ...
  const dele = async (id, pid, file_path1) => {
    ...
    console.log(file_path1);
    await axios.post(
      `/delete_img_Administration_fac/${id}`,
      { file_path1: file_path1, pid: pid },
      {
        method: "POST",
      }
  ...
  const handleSubmit_data = async (id, para) => {
    try {
      if (para !== "") {
        ...
        console.log(arr);
        await fetch(`/Administration_add_para/${id}`, {
          method: "POST",
          body: JSON.stringify(arr),
          headers: { "Content-Type": "application/json" },
        });
  ...
  const handleSubmit_link = async (id, link) => {
    try {
      console.log(link);
      await axios.post(`/Administration_add_link/${id}`, {
        link: link,
      });
      setCaption("");
      setAuth(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        // if (file) {
        setErrMsg("");
        await axios.post(`/Administration_upload`, {
          title: link,
          description: caption,
        });
        setCaption("");
