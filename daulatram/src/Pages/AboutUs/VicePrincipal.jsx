...
const VicePrincipal = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("http://localhost:5000/VicePrincipal_About");
  ...
  const del = async (id, pid, type) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/delete_VicePrincipal_About_data/${id}`,
        {
          method: "POST",
  ...
  const handleSubmit = async (files) => {
    try {
      if (files) {
        ...
        setErrMsg("");
        await axios.post(
          `http://localhost:5000/VicePrincipal_About_add`,
          { file: files },
          {
            headers: {
  ...
  const handleSubmit_data = async (id) => {
    try {
      if (para !== "") {
        ...
        console.log(arr);
        await fetch(`http://localhost:5000/VicePrincipal_About_add_data/${id}`, {
          method: "POST",
          body: JSON.stringify(arr),
          headers: { "Content-Type": "application/json" },
        });
