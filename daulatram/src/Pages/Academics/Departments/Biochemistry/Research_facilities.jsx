...
import Research_facilities_dat from "./Research_facilities_dat";
...
function Research_facilities() {
  ...
  const fetchdata = async () => {
    const response = await fetch("/Bio_Research_facilities");
  ...
  const del_para = async (id, pid, type) => {
    try {
      ...
      const response = await fetch(
        `/delete_Bio_Research_facilities_para/${id}`,
        {
          method: "POST",
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(`/delete_Bio_Research_facilities/${id}`, {
      method: "POST",
    });
    await response.json();
    if (response.status === 200) {
  ...
  const delete_file = async (id, pid, file_path1) => {
    ...
    await axios.post(
      `/delete_pdf_link_Bio_Research_facilities_fac/${id}`,
  ...
  const dele = async (id, pid, file_path1) => {
    ...
    console.log(file_path1);
    await axios.post(
      `/delete_img_Bio_Research_facilities_fac/${id}`,
      { file_path1: file_path1, pid: pid },
      {
        method: "POST",
      }
  ...
  const handleSubmit_data = async (id, para) => {
    try {
      if (para !== "") {
        ...
        console.log(arr);
        await fetch(`/Bio_Research_facilities_add_para/${id}`, {
          method: "POST",
          body: JSON.stringify(arr),
          headers: { "Content-Type": "application/json" },
        });
  ...
  const handleSubmit_link = async (id, link) => {
    try {
      console.log(link);
      await axios.post(`/Bio_Research_facilities_add_link/${id}`, {
        link: link,
      });
      setCaption("");
      setAuth(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        // if (file) {
        setErrMsg("");
        await axios.post(`/Bio_Research_facilities_upload`, {
          title: link,
          description: caption,
        });
        setCaption("");
