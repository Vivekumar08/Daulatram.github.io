...
function Awards() {
  ...
  const fetchdata = async () => {
    const response = await fetch("/Com_Awards");
  ...
  const del_para = async (id, pid, type) => {
    try {
      ...
      console.log(id, arr);
      const response = await fetch(`/delete_Com_Awards_para/${id}`, {
        method: "POST",
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(`/delete_Com_Awards/${id}`, {
      method: "POST",
    });
    await response.json();
    if (response.status === 200) {
  ...
  const delete_file = async (id, pid, file_path1) => {
    ...
    await axios.post(
      `/delete_pdf_link_Com_Awards_fac/${id}`,
  ...
  const handleSubmit_data = async (id, para) => {
    try {
      if (para !== "") {
        ...
        console.log(arr);
        await fetch(`/Com_Awards_add_para/${id}`, {
          method: "POST",
          body: JSON.stringify(arr),
          headers: { "Content-Type": "application/json" },
        });
  ...
  const handleSubmit_link = async (id, link) => {
    try {
      console.log(link);
      await axios.post(`/Com_Awards_add_link/${id}`, {
        link: link,
      });
      setCaption("");
      setAuth(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        // if (file) {
        setErrMsg("");
        await axios.post(`/Com_Awards_upload`, {
          title: link,
          description: caption,
        });
        setCaption("");
