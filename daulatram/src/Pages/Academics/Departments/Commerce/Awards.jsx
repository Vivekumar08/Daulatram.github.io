...
const Awards = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("http://localhost:5000/Com_Awards");
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `http://localhost:5000/delete_Com_Awards/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        await axios.post(`http://localhost:5000/Com_Awards_add`, formData, {
          headers: {
