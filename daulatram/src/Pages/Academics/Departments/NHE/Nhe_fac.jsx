...
import NHE_fac_data from "./NHE_fac_data";
import NHE_fac_data_sup from "./NHE_fac_data_sup";
...
const NHE_faculty = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("http://localhost:5000/NHE_faculty");
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `http://localhost:5000/delete_NHE_faculty/${id}`,
      {
        method: "POST",
      }
    );
  ...
  const handleSubmit_cv = async (id, pdf) => {
    ...
    try {
      ...
      if (pdf) {
        // setErrMsg("");
        await axios.post(
          `http://localhost:5000/NHE_faculty_cv_upload/${id}`,
          {
            file: pdf,
          },
          {
      ...
      if (link.trim() !== "" && caption.trim() !== "") {
        if (imag) {
          ...
          const data = await axios.post(
            `http://localhost:5000/NHE_faculty_file_upload`,
            // { method: "POST" },
