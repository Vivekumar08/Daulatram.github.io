...
const Physics_publications = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("http://localhost:5000/Physics_Publications");
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `http://localhost:5000/delete_Physics_Publications/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        await axios.post(`http://localhost:5000/Physics_Publications_add`, formData, {
          headers: {
