...
const Physics_events = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("http://localhost:5000/Physics_Events");
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `http://localhost:5000/delete_Physics_Events/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        await axios.post(`http://localhost:5000/Physics_Events_add`, formData, {
          headers: {
