...
const Pubs = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("http://localhost:5000/Eco_Pubs");
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `http://localhost:5000/delete_Eco_Pubs/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        await axios.post(`http://localhost:5000/Eco_Pubs_add`, formData, {
          headers: {
