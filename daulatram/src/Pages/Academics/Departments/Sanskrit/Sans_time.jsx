...
const Sans_time = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("http://localhost:5000/Sanskrit_Time");
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `http://localhost:5000/delete_Sanskrit_Time/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        await axios.post(`http://localhost:5000/Sanskrit_Time_add`, formData, {
          headers: {
