...
const Sans_publications = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("http://localhost:5000/Sanskrit_Publications");
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `http://localhost:5000/delete_Sanskrit_Publications/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        await axios.post(`http://localhost:5000/Sanskrit_Publications_add`, formData, {
          headers: {
