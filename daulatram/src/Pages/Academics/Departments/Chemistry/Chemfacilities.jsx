...
const Awards = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("http://localhost:5000/Chem_Fac");
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `http://localhost:5000/delete_Chem_Fac/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        await axios.post(`http://localhost:5000/Chem_Fac_add`, formData, {
          headers: {
