...
function Chem_Gallery() {
  ...
  const fetchdata = async () => {
    const response = await fetch("http://localhost:5000/Chem_Photo_Gallery");
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `http://localhost:5000/delete_Chem_Photo_Gallery/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (file) {
        ...
        setErrMsg("");
        await axios.post(`http://localhost:5000/Chem_Photo_Gallery_add`, formData, {
          headers: {
