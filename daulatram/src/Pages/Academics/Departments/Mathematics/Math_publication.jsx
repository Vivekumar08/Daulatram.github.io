...
const Publication = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("http://localhost:5000/Math_Publication");
  ...
  const del = async (id) => {
    ...
    const response = await fetch(
      `http://localhost:5000/delete_Math_Publication/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (link.trim() !== "" && caption.trim() !== "") {
        ...
        await axios.post(`http://localhost:5000/Math_Publication_add`, formData, {
          headers: {
