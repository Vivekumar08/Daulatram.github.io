...
const Tender = () => {
  ...
  const fetchdata = async () => {
    const response = await fetch("/Tender_Footer");
  ...
  const del = async (id) => {
    console.log(id);
    const response = await fetch(`/delete_Tender_Footer/${id}`, {
      method: "DELETE",
    });
    ...
    try {
      if (caption.trim() !== "") {
        if (!date_exp || !month_exp || !year_exp) {
          ...
          setErrMsg("");
          await axios.post(`/Tender_Footer_add`, formData, {
            headers: {
  ...
  const handleSubmit1 = async (e) => {
    ...
    try {
      if (!date_exp || !month_exp || !year_exp) {
        const date_e = null;
        setErrMsg("");
        const response = await fetch("/Tender_Footer_add_link", {
          method: "POST",
          headers: {
